...
const BLOCKER_RADIUS = {
  ...
  campfire: 30, tent: 45, logpile: 35,
};
...
export function createWorld() {
  ...
  const walls = [
    ...
    },
    {
      ...
      pad: { x: baseX + 540, y: baseY, radius: 60, cost: 400 },
  ...
  const nests = [
    ...
    { x: baseX + 1800, y: baseY, tier: 3, perNest: 1 },
  ];
  ...
  // 壁パッド(1532,900)まで最短約127、東氷壁(x=1612)のブロッカー余白22
  const crates = [
...
// 柵をまたぐ移動の中継点。内外をまたぐならゲートを経由させる。
export function routeViaGate(camp, fromX, fromY, toX, toY) {
  const fromIn = insideCamp(camp, fromX, fromY);
  const toIn = insideCamp(camp, toX, toY);
  if (fromIn === toIn) return { x: toX, y: toY };
  const gateY = camp.y2 + (fromIn ? -30 : 30);
  const nearGate =
    Math.abs(fromX - camp.gate.x) < camp.gate.halfW * 0.8 &&
    Math.abs(fromY - camp.y2) < 60;
  if (nearGate) return { x: toX, y: toY };
  return { x: camp.gate.x, y: gateY };
}
...
function nestSpawns(nests) {
  return nests.flatMap((n) =>
    Array.from({ length: n.perNest }, (_, i) => {
      if (n.perNest === 1) return { x: n.x, y: n.y, tier: n.tier };
      const a = (i / n.perNest) * Math.PI * 2 + n.x * 0.013;
      return { x: n.x + Math.cos(a) * 42, y: n.y + Math.sin(a) * 42, tier: n.tier };
    })
  );
}
...
function makeDecorations(width, height, camp, base, nests, zone, walls, lane, counter, blockers) {
  ...
  for (let i = 0; i < 11; i++) {
    ...
    if (cx < zone.x1 - 80 && rand() < 0.5) continue; // 到達不能な西側は疎らに
    for (let j = 0; j < 3; j++) {
      const x = cx + (rand() - 0.5) * 100;
      const y = cy + (rand() - 0.5) * 100;
  ...
  for (let i = 0; i < 66; i++) {
    ...
    if (x < zone.x1 - 80 && rand() < 0.55) continue; // 西側は疎らに
  ...
  const bands = [
    { x1: 60, y1: 60, x2: 350, y2: 1740, n: 34 },     // 西の到達不能帯
    { x1: 372, y1: 60, x2: 1612, y2: 330, n: 20 },    // zone0 北壁の外
    { x1: 372, y1: 1470, x2: 1612, y2: 1740, n: 20 }, // zone0 南壁の外
    { x1: 1612, y1: 60, x2: 3140, y2: 320, n: 22 },   // 東エリア北縁
    { x1: 1612, y1: 1480, x2: 3140, y2: 1740, n: 22 },// 東エリア南縁
    { x1: 2960, y1: 320, x2: 3140, y2: 1480, n: 14 }, // 最東端
  ];
  ...
  // campfire のみ拠点内 (940,860): cutboard パッド(860,920)と中心距離100 > r55+r30=85 を維持
